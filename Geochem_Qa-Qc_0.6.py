...
# def duplicates(geochem_data, element_list):
#     '''
#     This fuicntion is used to plot the duplicate pairs in order to compare
#     compare them over a range of concentraitons. The duplicates are found
#     using an index that is calculated earlier in the code. The plots are
#     output as both individual .PNG and as a compiled pdf.
...
#     # Trys to create a a folder to store the individual duplicate images
#     try:
#         os.mkdir(Save_Location + "\\Duplicates")
#     except FileExistsError:
#         print ("Folder already exists")
#     pdf = matplotlib.backends.backend_pdf.PdfPages(Save_Location +
#                                                    "\\Duplicates.pdf")
#     #Duplicate Statistics
#     temp_dupe_stats = np.zeros([len(element_list),4])
#     dupe_header = ('minimum', 'maximum', 'median', '90th percentile')
#     duplicate_stats = pd.DataFrame(data=temp_dupe_stats,index=element_list,
#                                    columns= dupe_header)
#     #try:
#     for i in range(0, len(element_list)):
...
#             geochem_data.columns).index(element_list[i])]
#         minimum, maximum, median, percentile = duplicate_statistics(x,y)
#         duplicate_stats.loc[element_list[i],dupe_header[0]] = minimum
#         duplicate_stats.loc[element_list[i],dupe_header[1]] = maximum
#         duplicate_stats.loc[element_list[i],dupe_header[2]] = median
#         duplicate_stats.loc[element_list[i],dupe_header[3]] = percentile
#         colour = []
#         for j in range(len(x)):
#             tx = list(x)
#             ty = list(y)
#             dif = abs((tx[j]-ty[j])/tx[j])
#             if dif < 0.1:
#                 colour = np.append(colour, 'k')
#             elif dif > 0.1 and dif < 0.15:
#                 colour = np.append(colour, 'b')
#             else:
#                 colour = np.append(colour, 'r')
#             # if dif < 1.1 and dif > 0.9:
#             #     colour = np.append(colour, 'k')
#             # elif dif > 1.1 and dif < 1.15:
#             #     colour = np.append(colour, 'b')
#             # elif dif < 0.9 and dif > 0.85:
#             #     colour = np.append(colour, 'b')
#             # else:
#             #     colour = np.append(colour, 'r')
#         plt.scatter(x, y, s= 3, color = colour)
#         # 1 to 1 line
#         plt.plot([0,99999],[0,99999],color = 'k',linestyle='dashed',
#                  linewidth=0.5)
#         #plus and minus 10% lines
#         plt.plot([0.0001,99999],[0.00011,(99999*1.1)],color = 'b',
#                  linestyle='dashed', linewidth=0.5)
#         plt.plot([0.0001,99999],[0.00009,(99999*0.9)],color = 'b',
#                  linestyle='dashed', linewidth=0.5)
#         #plus and minus 15% lines
#         plt.plot([0.0001,99999],[0.000115,(99999*1.15)],color = 'r',
#                  linestyle='dashed', linewidth=0.5)
#         plt.plot([0.0001,99999],[0.000085,(99999*0.85)],color = 'r',
#                  linestyle='dashed', linewidth=0.5)
#         savename = ('{}\\Duplicates\\{}_Duplicates.png').format(
#             Save_Location,element_list[i])
...
#         plt.clf()
#     pdf.close()
#     duplicates_save = ('{}\\Duplicate_Stats.xlsx').format(Save_Location)
#     try:
#         duplicate_stats.to_excel(duplicates_save)
#     except xlsxwriter.exceptions.FileCreateError:
#         print ('''Unable to save, the file must already be open. A new verison
#                will try to be saved with _new. If this fails no file will be
#                saved.''')
#         standards_save = ('{}\\Duplicate_Stats_new.xlsx').format(Save_Location)
#         duplicate_stats.to_excel(standards_save)
#     #except:
#       #  print ('No repeats Found')
#     del pdf

# def duplicate_statistics(repeat, repeat_pair):
#     '''
#     Function to calculate the statistics for pairs of duplicate samples. The
#     mean, standard deviation, and RSD are calculated for each duplicate pair.
#     Summary statistics are then calculated for the RSDs, these are: minimum,
#     maximum, median , and the 90th percentile.

#     Parameters
#     ----------
#     repeat : array
#         An array carrying the duplicate values for a sample.
#     repeat_pair : array
#         An array carrying the paired values for the repeat array.

#     Returns
#     -------
#     minimum : float
#         The minimum RSD value..
#     maximum : float
#         The minimum RSD value..
#     median : float
#         The median RSD value.
#     percentile : float
...
#     stats = np.zeros([number_repeats, 3])
#     for i in range(number_repeats):
#         stats[i,0] = st.mean([repeat[i], repeat_pair[i]])
#         stats[i,1] = st.stdev([repeat[i], repeat_pair[i]])
#         stats[i,2] = stats[i,1]/stats[i,0]
#     minimum = min(stats[:,2])
#     maximum = max(stats[:,2])
#     median = np.median(stats[:,2])
#     percentile = np.percentile(stats[:,2], 90)
#     return minimum, maximum, median, percentile
